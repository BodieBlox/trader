...
var values = [
  1, 2, 5, 10, 100, 250, 530, 1340, 5800, 24000, 82000, 450000, 1000000, 4500000, 14000000, 53000000, 160000000,
  999999999
]
var resources = []
var i = 0
...
function main() {
  ...
  // DOM
  $('body').append(
    "<div id='game'><img src='webh5games.jpg' id='splash'></img><div id='resourceWrap'></div><div id='upgradeWrap'></div>"
  )
    ...
    $row.append(
      $(
        "<td class='dealCell" +
          j +
          "' " +
          (j >= cols ? "style='visibility: hidden'" : '') +
          " onclick='makeTrade(this)'><div class='progress'></div></td>"
      )
    )
  }
...
function autoRes(res, ms) {
  ...
  window.setTimeout(function () {
    autoRes(res, resources[res].auto)
  }, ms)
}
...
function makeInventoryTable() {
  ...
  for (var i = 0; i < Math.min(bestRes + 2, resources.length); ++i) {
    $a.append(
      '<td' +
        (resources[i].manual > 0 ? " onclick='manualGet(" + i + ', ' + resources[i].manual + ")' class='manual'" : '') +
        '>' +
        "<div class='resVal'>" +
        (bestRes < 4 ? 'Value: ' : '') +
        p(resources[i].value) +
        '</div>' +
        "<img class='resName' id='" +
        resources[i].name +
        "' src='" +
        resources[i].name +
        ".png'><br>" +
    ...
    if (i % 6 === 5) $a.append('<tr>')
    first = false
  }

  $a.append(
    "<tr><td colspan=10>Score: <span id='score'>0</span>" +
      "<tr><td colspan=10 id='stats'>" +
      "Manual score: <span id='manualScore'>" +
      p(manualScore) +
      '</span>' +
      "; Trading score: <span id='tradingScore'>" +
      p(tradingScore) +
      '</span>' +
      "; Auto score: <span id='autoScore'>" +
      p(autoScore) +
      '</span>'
  )
  $table.append($a)
...
function manualGet(res, amt) {
  var name
  if (res === 0) name = 'coin'
  else if (res === 1) name = 'chop'
  else if (res === 2) name = 'brick'
  else if ((res > 2 && res < 8) || res === 13 || res === 15) name = 'clang'
  else name = 'ding'
...
function createFloatText(s, x, y) {
  ...
  $a.css('top', y)
    .css('left', x)
    .css('color', s > 0 ? '#006600' : '#660000')

  $('#game').append($a)
  window.setTimeout(function () {
    $a.css('top', y - 40 * Math.sign(+s)).css('opacity', 0)
  }, 0)
  window.setTimeout(function () {
    $a.remove()
  }, 2000)
...
function randomDeal() {
  ...
  do {
    var buyIndex = Math.max(
      0,
      Math.min(resources.length - 1, bestRes + 1, sellIndex - 3 + Math.floor(Math.random() * 7))
    )
  } while (Math.abs(buyIndex - sellIndex) === 0)
  var sellRes = resources[sellIndex]
  var buyRes = resources[buyIndex]
  var sell
  if (onlyAffordable) sell = Math.floor(Math.random() * sellRes.amt) + 1
  else sell = Math.min((GLOBAL_CAP + 1) / 10 - 1, Math.max(1, Math.ceil((Math.random() + 0.2) * sellRes.amt)))

  var sellVal = sell * sellRes.value
  var buy = Math.min(
    (GLOBAL_CAP + 1) / 10 - 1,
    Math.round((sellVal / buyRes.value) * (tradeMinRatio + (tradeMaxRatio - tradeMinRatio) * Math.random()))
  )
  if (buy === 0) {
    buy = 1
...
function createDeal(q, sellIndex, buyIndex, sell, buy) {
  ...
  img1.src = '' + sellRes.name + '.png'
  var img2 = new Image(24, 24)
  img2.src = '' + buyRes.name + '.png'
  var $div1 = $(
    "<div class='sell'>" +
      "<span class='sellAmt'>" +
      sell +
      ...
      resources.indexOf(sellRes) +
      '>' +
      '</div>'
  ).append(img1)
  if (helpValue) $div1.append($("<div class='helpValue'>" + p(sellRes.value * sell) + '</div>'))
  var $div2 = $(
    "<div class='buy'>×<span class='buyAmt'>" +
      buy +
      "</span><input type='hidden' value=" +
      resources.indexOf(buyRes) +
      '>' +
      (helpValue ? "<div class='helpValue'>" + p(buyRes.value * buy) + '</div>' : '') +
      '</div>'
  )
  $(img2).prependTo($div2)
...
function updateBgs() {
  ...
  for (var i = 0; i < upgrades.length; ++i) {
    ...
    if (typeof upgrade !== 'undefined') {
      ...
      if (upgrade.canBuy()) div.addClass('bgGreen')
      else div.addClass('bgRed')
    }
  }
}
...
function save() {
  ...
  localStorage.setItem(
    'tradingSave',
    JSON.stringify({ resources: resources, upgrades: boughtUps, aS: autoScore, tS: tradingScore, mS: manualScore })
  )
}
...
function totalSPS() {
  var sps = 0
  for (var i = 0; i < resources.length; ++i) {
    if (resources[i].auto > 0) sps += (1000 / resources[i].auto) * resources[i].value
  }
  return sps
}
...
function fillDiv(div) {
  ...
  div.css({
    ...
    top: '0px',
    '-webkit-transform':
      'translate(' + translationX + 'px, ' + translationY + 'px) scale3d(' + scaleX + ', ' + scaleX + ', 1)',
